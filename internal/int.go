...
// compileInt compiles the validation of a signed integer against the tags.
func compileInt(tags []string) []step {
	required := false
	hasDefault := false
	var def int64
	for _, t := range tags {
		if t == "notzero" {
			...
			v, err := strconv.ParseInt(t[len("default="):], 10, 64)
			if err != nil {
				continue
			}
			def = v
